...
namespace Canducci.Pagination.Mvc
{
    public static class HtmlHelpers
    {
        ...
        internal static HtmlContentBuilder TotalPages<T>(HtmlContentBuilder content, IPaginated<T> paginated, Func<int, string> generatePageUrl, PaginatedOptions options)
        {
            if (options.TotalPages)
            {                
                TagBuilder tag = TagLink("#", $"{(paginated.PageCount > 1 ? paginated.PageCount + " " + options.TotalPagesLabel : string.Empty)}", options.CssClassAnchor);
                content.AppendHtml(tag, options.CssClassLiDisabled + " " + options.CssClassLi);
            }
            return content;
        }
        #endregion
        ...
        internal static IHtmlContent PaginationNumbersWithFirstPreviousNextLast<T>(IPaginated<T> paginated, Func<int, string> generatePageUrl, PaginatedOptions options = null)
        {
            ...
            TotalPages(content, paginated, generatePageUrl, options);
